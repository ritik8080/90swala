...
// ------------------------------------------------------------
const YOUTUBE_PLAYLIST_ID = "PLuRJywfkv_0zEScfeRiK22sXe6y_eO2q8";
const USE_YOUTUBE_PLAYLIST = false;
...
function playIndex(i, { count = true } = {}) {
  ...
  if (USE_YOUTUBE_PLAYLIST && player.yt?.getPlaylist && player.yt?.playVideoAt) {
    ...
    } else if (player.yt?.playVideoAt) {
      // Fallback: the catalog video is not in the supplied playlist, 
      // but we still want the playlist to run, so start from the beginning.
      player.yt.playVideoAt(0);
    }
